...
def client(hostname, port):
    ...
    while True:
        sock.send(data)
        print('Waiting up to {} seconds for reply'.format(delay))
        sock.settimeout(delay)
        try:
            data = sock.recv(MAX_BYTES)
        except socket.timeout:
            delay *=2 # wait even longer for the next reply
            if delay > 2.0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('host', help='interface the server listen at;''host the clent sends to')
